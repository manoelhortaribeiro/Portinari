...
class TestGraphMaker(unittest.TestCase):
    ...
    def complex_graph():
        ...
        edges = [
            {'source': 'n0', 'className': 'Edge', 'display_value': [],
             'dst': nodes[1],
             'destination': 'n1', 'key_op_value': [],
             'src': nodes[0],
             'id': 2, 'name': 'e2', 'kind': 'directed'},

            {'source': 'n3', 'className': 'Edge', 'display_value': [],
             'dst': nodes[1],
             'destination': 'n1', 'key_op_value': [],
             'src': nodes[2],
             'id': 4, 'name': 'e4', 'kind': 'directed'},

            {'source': 'n5', 'className': 'Edge', 'display_value': [],
             'dst': nodes[4],
             'destination': 'n7', 'key_op_value': [],
             'src': nodes[3],
             'id': 8, 'name': 'e8', 'kind': 'directed'},

            {'source': 'n5', 'className': 'Edge', 'display_value': [],
             'dst': nodes[5],
             'destination': 'n9', 'key_op_value': [],
             'src': nodes[3],
             'id': 10, 'name': 'e10', 'kind': 'directed'},

            {'source': 'n1', 'className': 'Edge',
             'display_value': ['Time Interval smaller than 365'],
             'dst': nodes[3],
             'destination': 'n5',
             'key_op_value': [['TimeSinceLast', '<', '365']],
             'src': nodes[1],
             'id': 11, 'name': 'e11', 'kind': 'undirected'}
        ]
